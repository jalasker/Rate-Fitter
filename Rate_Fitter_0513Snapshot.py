...
class Rate_Fitter:
    ...
    def fit_rate(self, fixK = False, fixBeta = False, simIndex = 100, trueBeta = 0, CCScale = 2.0):
        ...
        print "NCC Before"
        print NCC
        #NCC = 2.0*NCC #applyCCScale(NCC, CCScale)
        print "NCC After"
        print NCC
        ...
        try:
            ...
            if simIndex < self.nprint:
                ...
                print "Mean NCC/NData"
                print np.mean(TrueNCC*1.0/(1.0*nData))
        except:
        ...
        for simInd in xrange(1,nfile):
            ...
            if simLoaded:
                try:
                    ...
                    RateTest.fit_rate(fixK = fixK, fixBeta = fixBeta, simIndex = simInd, trueBeta = trueBeta - 2.11, CCScale = 1.0)
                    '''
                    ...
                    RateTest.fit_rate(fixK = fixK, fixBeta = fixBeta, simIndex = simInd, trueBeta = trueBeta - 2.11, CCScale = CCScale)
                    CCScale, CCScaleErr =  getCCScale(RateTest.simcat.Catalog, RateTest.realcat.Catalog, MURESWindow = (ScaleMuResCutLow, ScaleMuResCutHigh), zbins = scaleZBins, muresBins = muresBins, Beta = RateTest.Beta, binList = RateTest.binList, fracCCData = RateTest.fracCCData, outfilePrefix =  dataname)
                    RateTest.fit_rate(fixK = fixK, fixBeta = fixBeta, simIndex = simInd, trueBeta = trueBeta - 2.11, CCScale = CCScale)
                    '''
                ...
                try:
                    ...
                    '''
                    CCScale, CCScaleErr =  getCCScale(RateTest.simcat.Catalog, RateTest.realcat.Catalog, MURESWindow = (ScaleMuResCutLow, ScaleMuResCutHigh), zbins = scaleZBins, muresBins = muresBins, Beta = RateTest.Beta, binList = RateTest.binList, fracCCData = RateTest.fracCCData, outfilePrefix =  dataname)
                    RateTest.fit_rate(fixK = fixK, fixBeta = fixBeta, trueBeta = trueBeta - 2.11, CCScale = CCScale)
                    CCScale, CCScaleErr =  getCCScale(RateTest.simcat.Catalog, RateTest.realcat.Catalog, MURESWindow = (ScaleMuResCutLow, ScaleMuResCutHigh), zbins = scaleZBins, muresBins = muresBins, Beta = RateTest.Beta, binList = RateTest.binList, fracCCData = RateTest.fracCCData, outfilePrefix =  dataname)
                    RateTest.fit_rate(fixK = fixK, fixBeta = fixBeta, trueBeta = trueBeta - 2.11, CCScale = CCScale)
                    '''
